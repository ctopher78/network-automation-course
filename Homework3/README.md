...
## Yang Explorer
Cisco has also released a web-based [YANG explorer](https://developer.cisco.com/codeexchange/github/repo/CiscoDevNet/yang-explorer/), which greatly helps with navigating and making sense of a particular yang model.  I found this hugely helpful while I was navigating the yang tree in the Python bindings, which isn't always intuitive.
...
**NOTE:** This YDK tooling is executed from a vagrant box, not your local machine; however, the initial provisioning of the lab linux server and vqfx devices requires `ansible` and `junos-eznc` be installed locally -- which is installed into a virtualenv during the `pipenv install` portion below.
